...
export const ParticipantDetailsModal: FC<Props> = ({ person, isOpen, onClose, onUpdate }) => {
    ...
    const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!uploadError) {
            ...
            // Atualiza no banco de dados a coluna photoUrl
            ...
            if (updateError) {
                ...
                setLocalPerson(prev => prev ? { ...prev, photoUrl: publicUrl } : null);
            }
        } else {
    ...
    const handleRemovePhoto = async () => {
        ...
        if (localPerson.photoUrl) {
            const filePath = localPerson.photoUrl.split('/').slice(-2).join('/');
        ...
        setLocalPerson((prev) => prev ? { ...prev, photoUrl: '' } : null);
        setPreviewUrl(null);
    };
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Detalhes do Participante" size="xxl">
            <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="flex-1 text-sm space-y-2">
                    <p><span className="font-medium text-gray-600">Nome:</span> {localPerson.name}</p>
                    <p><span className="font-medium text-gray-600">E-mail:</span> {localPerson.email || '—'}</p>
                    <p><span className="font-medium text-gray-600">Telefone:</span> {localPerson.phone || '—'}</p>
                    <p>
                        <span className="font-medium text-gray-600">Status de Envio:</span>{' '}
                    ...
                    <p>
                        <span className="font-medium text-gray-600">Status de Leitura:</span>{' '}
                        <span className={localPerson.read ? 'text-blue-600 font-semibold' : 'text-gray-400 font-semibold'}>
